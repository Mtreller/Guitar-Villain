...
function render(dt){
  ...
  // camera: subtle beat bob + miss shake
  ...
  drawPops(dt);
  ctx.restore();
  drawProgress(); // unaffected by shake
}
...
function drawBackground(dt){
  ...
  // horizon glow band (notes emerge from here)
...
function drawHighway(alpha){
  ...
  const ty=G.horizonY, sYtop=G.strikeY;
  ...
  // main deck (glossy gradient, extends below strike for reflection zone)
  ...
  // per-lane tint + press/hold glow beams
  ...
  // converging lane divider lines (fade toward horizon = depth fog)
  for(let i=0;i<=LANES;i++){
  ...
  // scrolling floor rungs (perspective, continuous) -> strong 3D motion
  const t=audioTime();
  const spacing=0.5; // seconds between rungs
...
function drawSideRails(alpha){
  ...
  [[farL,nearL,c1],[farR,nearR,c2]].forEach(([fx,nx,c])=>{
    // outer bloom
    ctx.shadowColor=c; ctx.shadowBlur=PERF_MOBILE?6:(18+14*energy);
...
/* shadows cast by notes onto the deck (drawn before notes) */
...
/* 3D hold tube with flowing highlight */
function drawTail(lane, headFrac, tailFrac, n){
  ...
  // glowing cap at the far end of the hold
...
/* 3D puck-style note with side, top face, specular + rim glow */
function drawGem(lane, frac, n){
  ...
  const rt=r*0.72;                    // vertical radius (ellipse)
  const thick=r*0.42;                 // puck thickness
  const col=LANE_COL[lane];
  ...
  // outer glow
  ctx.shadowColor=col; ctx.shadowBlur=(14+10)*s;

  // bottom rim (underside)
  ctx.beginPath(); ctx.ellipse(x,y+thick,r,rt,0,0,Math.PI*2);
  ctx.fillStyle=shadeA(col,-75,0.95); ctx.fill();
  ctx.shadowBlur=0;
  // side band (straight walls between top and bottom ellipse)
  ctx.beginPath();
...
function drawFrets(alpha){
  ...
  for(let i=0;i<LANES;i++){
    ...
    ctx.shadowColor=col; ctx.shadowBlur=PERF_MOBILE?(active?8:3):(active?(24+20*hg):10*pulse); ctx.stroke();
    ctx.shadowBlur=0;

    // inner fill (glossy dome)
...
function drawParticles(dt){
  for(let i=particles.length-1;i>=0;i--){
    ...
    ctx.shadowColor=p.col; ctx.shadowBlur=PERF_MOBILE?0:8;
...
function drawPops(dt){
  ...
  for(let i=pops.length-1;i>=0;i--){
    ...
    ctx.shadowColor=p.col; ctx.shadowBlur=PERF_MOBILE?0:14;
